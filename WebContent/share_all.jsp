...
{
	out.print("<script>alert('Share Request send to Admin')</script>");
}
...
//            id_k = rss.getInt("id");
        ...
        ResultSet rss1 = stt1.executeQuery("select * from n_file where User='"+s_user+"'");  
            while(rss1.next()){
                int id = rss1.getInt("u_id");
//                if(id_k == id){
                    String filename = rss1.getString("File_Name");
                    Statement ss = con.createStatement();
                    ResultSet rrr = ss.executeQuery("select * from share_all where u_id="+id+" and status = 'true'");
                    if(rrr.next()){
                    %>
					<tr>
						<td style="text-align: center; border: 1px solid"><%=filename%>
						</td>
						<td style="text-align: center; border: 1px solid">Share To
							All</td>
						<td style="text-align: center; border: 1px solid"><a
							href="share_a.jsp?id=<%=id%>&file=<%=filename%>&user=<%=s_user%>&privilege=<%=pri%>">Share
								Request</a></td>
